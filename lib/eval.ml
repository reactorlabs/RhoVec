...
let is_zero_subsetting (l : int option list) =
...
let is_negative_subsetting (l : int option list) =
  ...
  List.for_all
    (function
      | None -> true (* Let this go for now, we catch the error later *)
      | Some i when i <= 0 -> true
      | Some _ -> false)
    l
...
let neg_vec_to_bool (l : int option list) n =
  ...
  List.iter
    (function
      ...
      | Some i when i >= 0 -> ()
      | Some _ | None -> raise Mixing_with_negative_subscripts)
    l' ;
  a

let update_at_pos (a1 : literal array) (l2 : int option list) (l3 : literal list) ty =
  ...
  let max_idx =
    List.fold_left
      (fun n x ->
        ...
        | None -> n)
      len l2 in
  let a1' = extend a1 max_idx ty in
  List.iter2
    (fun n x ->
      match n with
      | Some i when 1 <= i -> a1'.(i - 1) <- x
      | Some _ -> raise Mixing_with_negative_subscripts
      | None -> ())
    l2 l3 ;
  a1'
...
let rec eval e =
  ...
  | Subset1_Nothing_Assign (e1, e2) ->
      ...
      if m = 0 then raise Replacement_length_is_zero ;
  ...
  | Subset1_Assign (e1, e2, e3) -> (
      ...
      | T_Bool ->
          ...
          let res = update_at_pos a1' l2' l3 t1 in
          Vector (res, t1)
      | T_Int ->
          let l2 = Array.to_list (Array.map extract_int a2) in
          if is_zero_subsetting l2 then base
          else if is_positive_subsetting l2 then (
            if Array.exists is_na a2 then
              (* We diverge from R and ban NAs as indices during assignment *)
              raise No_NAs_in_subscripted_assignment ;
  ...
  | Subset2 (e1, e2) -> (
      ...
      let (Vector (a2, t2)) = eval e2 in
      let n = Array.length a2 in
      if n < 1 then raise Selecting_lt_one_element ;
      ...
      | Some n when n < 0 -> raise Invalid_negative_subscript
  ...
  | Subset2_Assign (e1, e2, e3) -> (
      ...
      if n < 1 then raise Selecting_lt_one_element ;
      if n > 1 then raise Selecting_gt_one_element ;
      if m < 1 then raise Replacement_length_is_zero ;
